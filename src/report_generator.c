...
/*  Δήλωση συνάρτησης generateReport() με παραμέτρους double cpu_time_used, 
    ...
    το οποίο περιέχει τα αποτελέσματα από την κάθε εκτέλση του mycp. Αν αποτύχει
    η δημιουργία του εκτυπώνεται αντίστοιχο μήνυμα σφάλματος και ο κωδικός του.*/

int generateReport(double cpu_time_used, int buffer_size, long file_size) {
    ...
    fprintf(report_file, "Time taken: %f seconds | Buffer size: %d | File size: %ld bytes \n", cpu_time_used, buffer_size, file_size);
